...
class SecondMainWindowThread(QThread):
    def run(self):
...
class Canvas_grafica(FigureCanvas):
    ...
    def grafica_datos(self):
        ...
        self.ax.grid(True)
        self.draw()
class myMainwindow(qw.QMainWindow, GUI.Ui_MainWindow):
    def __init__(self):
        super().__init__()
        # ui = ui_uart_tools.Ui_MainWindow()
        self.setupUi(self)
        self.second_window_thread = SecondMainWindowThread()
        ...
        self.btn_send_5.clicked.connect(self.btn_send_5_cb)
        self.btn_send_6.clicked.connect(self.btn_send_6_cb)
        ...
        self.statusbar.showMessage("status: INIT 1")
    ...
    def btn_send_5_cb(self):
        if self.uart_connected:
            ...
            time_str = current_time.strftime("%Y-%m-%d %H:%M:%S")
            # Tạo chuỗi thông báo mới có thêm thông tin thời gian
            message = f"{time_str}: check1 copy"
            # Hiển thị thông báo trong textBrowser_2
            self.textBrowser_2.append(message)
            amp_text = self.AMP.toPlainText()
            freq_text = self.FREQ.toPlainText()
            # Tạo chuỗi thông báo mới có thêm thông tin thời gian
            message = f"{amp_text} {freq_text}"
            # Hiển thị thông báo trong textBrowser_2
            self.textEdit_get.append(message)
        else:
            ...
            time_str = current_time.strftime("%Y-%m-%d %H:%M:%S")
            # Tạo chuỗi thông báo mới có thêm thông tin thời gian
            message = f"{time_str}: UART NO CONNECTED"
            # Hiển thị thông báo trong textBrowser_2
            self.textBrowser_2.append(message)
